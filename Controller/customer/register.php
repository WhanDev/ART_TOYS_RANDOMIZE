...
<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST") {   
        ...
        $conf_user_password = trim($json_data["conf_user_password"]);
        $user_role = "customer";
    } else { 
...
<?php
    ...
    if ( empty($f_name) || empty($l_name) || empty($email) || empty($tel) || empty($address) || empty($user_password) || empty($conf_user_password)) {
        echo json_encode(array("result" => 0, "messages" => "กรุณากรอกข้อมูลให้ครบถ้วน"));
        exit;
    } else {
        if ($user_password != $conf_user_password) {
            echo json_encode(array("result" => 0, "messages" => "รหัสผ่านไม่ตรงกัน"));
            exit;
        }
